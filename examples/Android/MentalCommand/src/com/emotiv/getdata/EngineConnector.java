...
import com.emotiv.insight.IEdk.IEE_MentalCommandEvent_t;
...
public class EngineConnector {
	...
	public void enableMentalcommandActions(IEE_MentalCommandAction_t _MetalcommandAction) {
		...
		if (activeAction[0] == IEdkErrorCode.EDK_OK.ToInt()) {
			...
			if (y == 0) {
				...
				IEdk.IEE_MentalCommandSetActiveActions(userId, MetaCommandActions);
	...
	public boolean checkTrained(int action) {
		long[] result = IEdk.IEE_MentalCommandGetTrainedSignatureActions(userId);
	...
	public void trainningClear(int _MetalcommandAction) {
		IEdk.IEE_MentalCommandSetTrainingAction(userId, _MetalcommandAction);
		if (IEdk.IEE_MentalCommandSetTrainingControl(userId,
	...
	public boolean startTrainingMetalcommand(Boolean isTrain,
		...
		if (!isTrain) {
			if (IEdk.IEE_MentalCommandSetTrainingAction(userId,
	...
	public void setTrainControl(int type) {
		if (IEdk.IEE_MentalCommandSetTrainingControl(userId, type) == IEdkErrorCode.EDK_OK
				.ToInt()) {
		}
			...
			public void run() {
				...
				if (state == IEdkErrorCode.EDK_OK.ToInt()) {
					...
					case TYPE_METACOMMAND_EVENT:
						int type = IEdk.IEE_MentalCommandEventGetType();
