...
public class EngineConnector {
   ...
   public boolean startFacialExpression(Boolean isTrain,IEE_FacialExpressionAlgo_t FacialExpressionAction) {
		if (!isTrain) {
			if (IEdk.IEE_FacialExpressionSetTrainingAction(userId,FacialExpressionAction.ToInt()) == IEdkErrorCode.EDK_OK.ToInt()) {
				if (IEdk.IEE_FacialExpressionSetTrainingControl(userId,IEE_FacialExpressionTrainingControl_t.FE_START.getType()) == IEdkErrorCode.EDK_OK.ToInt()) {
					return true;
				}
			}
		} else {
			if (IEdk.IEE_FacialExpressionSetTrainingControl(userId,IEE_FacialExpressionTrainingControl_t.FE_RESET.getType()) == IEdkErrorCode.EDK_OK.ToInt()) {
				return false;
			}
		}
		return false;
   ...
   public void trainningClear(int _FacialdAction) {
		IEdk.IEE_FacialExpressionSetTrainingAction(userId, _FacialdAction);
		if (IEdk.IEE_FacialExpressionSetTrainingControl(userId,IEE_FacialExpressionTrainingControl_t.FE_ERASE.getType()) == IEdkErrorCode.EDK_OK.ToInt()) {
		}
	}
   public void setTrainControl(int type) {
		if (IEdk.IEE_FacialExpressionSetTrainingControl(userId, type) == IEdkErrorCode.EDK_OK.ToInt()) {
		}
	}
   public boolean checkTrained(int action){
		long[] result = IEdk.IEE_FacialExpressionGetTrainedSignatureActions(userId);
		if (result[0] == 0) {
...
//			    	   		
			    	   		case TYPE_FACIAL_EVENT:
			    	   			int type = IEdk.IEE_FacialExpressionEventGetType();
